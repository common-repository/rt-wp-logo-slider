    ...
    class rtWLSAjaxResponse
    {
        ...
        function rtWLSSaveSettings(){
            ...
            if($rtWLS->verifyNonce()){
                ...
                $error = true;
                $msg =__('Settings successfully updated', 'rt-wp-logo-slider');
            }else{

                    $msg = __('Security Error !!', 'rt-wp-logo-slider');
            }
            wp_send_json( array(
                'error'=> $error,
                'msg' => $msg
        ...
        function shortCodeList(){
            ...
            if ( $scQ->have_posts() ) {
                ...
                $html .= '<label class="mce-widget mce-label" style="padding: 20px;font-weight: bold;" for="scid">'.__('Select Short code', 'rt-wp-logo-slider').'</label>';
                $html .= "<select name='id' id='scid' style='width: 150px;margin: 15px;'>";
                $html .= "<option value=''>".__('Default', 'rt-wp-logo-slider')."</option>";
                ...
                $html .= "<div>".__('No shortcode found.','rt-wp-logo-slider')."</div>";
            }
            echo $html;
            die();
        }
